...
```{r}
vis_miss(autos) # No hay datos faltantes
vis_dat(autos)

autos <- autos[,1:8]
ggpairs(autos, columns = 1:8)
```
...
```{r}
autos
result <- sample.split(autos, SplitRatio = .7)
autos.train <- autos[result==TRUE, ]
autos.test <- autos[result==FALSE, ]
```
...
```{r}
#hist(log10(autos.train$disp))
#hist(log(autos.train$disp))
#hist(sqrt(autos.train$disp))
#hist(orderNorm(autos.train$disp)$x.t) # Ordered Quantile Normalization
hist(autos.train$disp)
autos.train$disp <- orderNorm(autos.train$disp)$x.t
hist(autos.train$disp)


#hist(log10(autos.train$weight))
#hist(log(autos.train$weight))
#hist(sqrt(autos.train$weight))
#hist(orderNorm(autos.train$weight)$x.t) # Ordered Quantile Normalization
hist(autos.train$weight)
autos.train$weight <- orderNorm(autos.train$weight)$x.t
hist(autos.train$weight)


#hist(log10(autos.train$mpg))
#hist(log(autos.train$mpg))
#hist(sqrt(autos.train$mpg))
#hist(orderNorm(autos.train$mpg)$x.t) # Ordered Quantile Normalization
hist(autos.train$mpg)
autos.train$mpg <- log10(autos.train$mpg)
hist(autos.train$mpg)


autos.test$disp <- orderNorm(autos.test$disp)$x.t
autos.test$weight <- orderNorm(autos.test$weight)$x.t
autos.test$mpg <- log10(autos.test$mpg)
```

```{r}
# reg <- lessR::reg(mpg ~ disp + weight, data=autos.train)
reg <- lm(formula = mpg ~ disp + weight, data = autos.train) # https://github.com/cran/lessR/blob/master/NEWS # Regression
```
...
```{r}
library(Metrics)
prediccion <- predict(reg, autos.test)
prediccion[1]
autos.test$mpg[1]
mltools::mse(preds = prediccion, actuals=autos.test$mpg)
```
