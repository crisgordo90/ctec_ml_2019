...
x[y == 1, ] = x[y == 1, ] + 1
plot(x, col = (3 - y))
dat = data.frame(x = x, y = as.factor(y))  # Convertimos a un dataframe y convertimos la y como factor (porque ocupamos un flag - Class)
...
# El mayor es que el que tenga mas area bajo la curva
# El training data esta ajustado para al modelo creado, mientras que el test no esta generalizado
```
...
# FITTED -> VALORES PREDICHOS y LABELS -> ETIQUETAS
precrec_obj2 <- evalmod(scores = fitted, labels = dat[-train, "y"], mode = "basic")
autoplot(precrec_obj2)   


# FITTED -> VALORES PREDICHOS y LABELS -> ETIQUETAS
precrec_obj2 <- evalmod(scores = fitted, labels = dat[-train, "y"])
autoplot(precrec_obj2)   
